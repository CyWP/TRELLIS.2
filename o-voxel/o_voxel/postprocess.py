...
def to_glb_geo(
    ...
    """
    Convert an extracted mesh to a GLB file with only geometry (no textures).
    Performs cleaning and optional remeshing.
    ...
    if voxel_size is not None:
        ...
        if isinstance(grid_size, (list, tuple, np.ndarray)):
            grid_size = torch.tensor(
                grid_size, dtype=torch.int32, device=vertices.device
            )
    ...
    if verbose:
        print(
            f"After filling holes: {mesh.num_vertices} vertices, {mesh.num_faces} faces"
        )
    ...
    if not remesh:
        ...
        if verbose:
            print(
                f"After remeshing: {mesh.num_vertices} vertices, {mesh.num_faces} faces"
            )
    ...
    if verbose:
        print(
            f"Final mesh: {len(out_mesh.vertices)} vertices, {len(out_mesh.faces)} faces"
        )
...
def to_glb(
    ...
    if not remesh:
        ...
        if verbose:
            print(
                f"After inital simplification: {mesh.num_vertices} vertices, {mesh.num_faces} faces"
            )
        ...
        if verbose:
            print(
                f"After initial cleanup: {mesh.num_vertices} vertices, {mesh.num_faces} faces"
            )
        ...
        if verbose:
            print(
                f"After final simplification: {mesh.num_vertices} vertices, {mesh.num_faces} faces"
            )
        ...
        if verbose:
            print(
                f"After final cleanup: {mesh.num_vertices} vertices, {mesh.num_faces} faces"
            )
    ...
    # --- Branch 2: Remeshing Pipeline ---
        ...
        if verbose:
            print(
                f"After simplifying: {mesh.num_vertices} vertices, {mesh.num_faces} faces"
            )

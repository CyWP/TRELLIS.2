...
class VarLenTensor:
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            ...
            ), f"Only integer indices are supported: {idx}"
...
class SparseTensor(VarLenTensor):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.SparseTensorData is None:
            ...
            if config.CONV == "torchsparse":
                self.SparseTensorData = importlib.import_module(
                    "torchsparse"
                ).SparseTensor
        ...
        self._shape = shape
        self._scale = kwargs.get(
            "scale", (Fraction(1, 1), Fraction(1, 1), Fraction(1, 1))
        )
    ...
    def replace(
        ...
        new_tensor = SparseTensor(
            new_data,
            shape=(
                torch.Size([self._shape[0]] + list(feats.shape[1:]))

...
def get_scene_geometry(
    scene: Union[trimesh.Trimesh, trimesh.Scene],
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Extract combined vertices and faces from a trimesh mesh or scene.

    Args:
        scene: A trimesh.Trimesh or trimesh.Scene containing geometry

    Returns:
        Tuple containing:
            - vertices: np.ndarray of shape (N, 3) with vertex positions (float32)
            - faces: np.ndarray of shape (M, 3) with triangle face indices (int32)
    """
    ...
    for mesh in groups:
        if len(mesh.vertices) > 0:
            all_vertices.append(np.asarray(mesh.vertices, dtype=np.float32))

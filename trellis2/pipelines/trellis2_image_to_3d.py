...
class Trellis2ImageTo3DPipeline(Pipeline):
    ...
    def sample_sparse_structure(
        ...
        decoded = decoder(z_s) > 0  # [1, 1, 64, 64, 64]
        # For testing voxel export
        # from trellis2.utils.debug_utils import vox2mesh

        # vox2mesh(decoded.squeeze(1), save_path="voxel_decoded_test.glb")
        # breakpoint()
        if self.low_vram:
            decoder.cpu()
        ...
        coords = torch.argwhere(decoded)[
            :, [0, 2, 3, 4]
        ].int()  # Just extracts voxel coords, ignores second dim (always 1 i guess)
    ...
    def sample_shape_slat_cascade(
        ...
        slat = self.shape_slat_sampler.sample(
            ...
            **sampler_params,
            verbose=True,
            tqdm_desc="Sampling shape SLat",
        ).samples  # Not upsampled either, still [3173, 4] for coords
        if self.low_vram:
            flow_model_lr.cpu()
        std = torch.tensor(self.shape_slat_normalization["std"])[None].to(slat.device)
        mean = torch.tensor(self.shape_slat_normalization["mean"])[None].to(slat.device)
    ...
    def run(
        ...
        cond_1024 = (
            self.get_cond([image], 1024) if pipeline_type != "512" else None
        )  # Approx [1, res, res] (got 1, 1029, 1024)
        ...
        # shape_slat: coords[num_voxels, 4] (although voxel count becomes higher than first coords. now [15536, 4])
        # shape_slat: feats[num_voxels, 32], so [15536, 32]
        # tex_slat: same shapes as shape_slat
        # res: same int resolution (1024)

        torch.cuda.empty_cache()
        out_mesh = self.decode_latent(shape_slat, tex_slat, res)
        # out-mash: MeshWiThvoxel object
        if return_latent:
